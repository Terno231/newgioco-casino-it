...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-dc1ff9cb";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-dc1ff9cb">'+'<span class="tile-face-dc1ff9cb tile-front-dc1ff9cb"></span>'+'<span class="tile-face-dc1ff9cb tile-back-dc1ff9cb"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-dc1ff9cb")){return;}
var back=tile.querySelector(".tile-back-dc1ff9cb");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-dc1ff9cb");if(isMine){tile.classList.add("boom-dc1ff9cb");}
if(dim){tile.classList.add("dimmed-dc1ff9cb");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-dc1ff9cb")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-dc1ff9cb");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-dc1ff9cb")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-dc1ff9cb");void panelEl.offsetWidth;panelEl.classList.add("jolt-dc1ff9cb");}
endRound();}
